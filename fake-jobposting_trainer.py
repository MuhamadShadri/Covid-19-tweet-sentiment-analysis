...
def model_builder(hp):
    ...
    x = layers.Embedding(VOCAB_SIZE, hp.get('embedding_dim', 16), name='embedding')(x)
    x = layers.Bidirectional(layers.LSTM(hp.get('lstm_units', 32)))(x)
    for _ in range(hp.get('num_layers', 1)):
        x = layers.Dense(hp.get('dense_units', 64), activation='relu')(x)
